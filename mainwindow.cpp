...
void MainWindow::signUpUser()
{
    ...
    if (name.isEmpty() || pass.isEmpty() || city.isEmpty() || addr.isEmpty() || phone.isEmpty()) {
        setStatus("Missing Information", true); return;
    }

    for(int i = 0 ; i<User::getUsersInSystem().vectorSize() ; i++)
    {Vector<User*> users = User::getUsersInSystem();
        if(phone.toStdString()==(users[i]->getUserContactNumber()))
        {setStatus("User Already Signed Up.",true); return;} }

    User::userSignUp(name.toStdString(), city.toStdString(),
...
void MainWindow::loginUser()
{
    ...
    bool login = User::userLogIn(id,liPass->text().toStdString());
        if(login==true){
        currentUser = User::searchForUser(id);
        ...
        setStatus(QString("Welcome, %1!").arg(QString::fromStdString(currentUser->getUserName())));
    } else {
...
void MainWindow::placeOrder()
{
    ...
    // build order
    Order *o       = new Order(currentUser->getUserID(),currentUser->getUserAddress(),0,shop->getShopID(),shop->getShopName());
    float total = 0;
    for (int i = 0; i < cart.vectorSize(); i++) {
...
{   static int loadCount = 0;
        if(loadCount!=0)
       setStatus("Data Already Loaded",true);
    else{
    FileManager::loadAllSystemData();
    FileManager::generateSystemSummaryReport();
    refreshUsersTable();
    refreshShopsTable();
    refreshProductsTable();
    refreshOrderQueue();
