...
void FileManager::loadAllSystemData() {
    ...
    if (uFile.is_open()) {
        ...
        while (getline(uFile, line)) {
            ...
            if (!id.empty())
            {new User(name,city,address,phone,password);}
    ...
    if (pFile.is_open()) {
        ...
        {stringstream ss(line);
            ...
            getline(ss, id, ','); getline(ss, shopname, ','); getline(ss, shopid, ','); getline(ss, name, ',');
            getline(ss, category, ','); getline(ss, price, ','); getline(ss, items);
            if (!id.empty())
    ...
    if (oFile.is_open()) {
        ...
        while (getline(oFile, line)) {
            stringstream ss(line);
            string userid,id, shopid, shopname, orderprice;

            getline(ss, id, ','); getline(ss, userid, ','); getline(ss, shopid, ',');
            getline(ss, shopname, ','); getline(ss, orderprice);
            ...
            if (!id.empty()) {
                ...
                User* u =  User::searchForUser(stoi(userid));
                cout<<stoi(userid)<<endl;
                u->addUserOrder(o);
                Shop* s = Shop::searchForShop(stoi(shopid));
                OrderDeque* d = s->getShopDeque();
                o->priority=Standard;
                d->pushRear(o);
